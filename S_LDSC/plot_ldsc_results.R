...
cardiovascular_traits = c("CAD_MVP_EUR_meta", "CAD_GWAS_van_der_Harst", "Myocardial_infarction", "CAC_GWAS", "Pulse_pressure_MVP",
                           "Type_2_diabetes")
...
level1_merged_df %>%
  ...
  coord_flip() + 
  facet_wrap(~gwas, scales = c("free_x")) + 
  custom_theme() + 
...
# Plot ldsc and magma results jointly as a bar plot 
level2_smc_merged_df %>%
  filter(gwas %in% cardiovascular_traits) %>%
  ggplot(aes(x=annotation, y=-log10(pvalue), fill=method)) + 
  geom_col(position = "dodge", width = 0.7) + 
  geom_hline(yintercept = 1.3, color="black", linetype="dashed") + 
...
# Plot results
myeloid_prioritization_df %>%
  filter(gwas %in% c("CAD_MVP_EUR_meta", "Pulse_pressure_MVP", 
                     "Alzheimer_disease_Jansen", "White_blood_cell_count")) %>%
  ggplot(aes(x=annotation, y=-log10(pvalue), color=annotation)) + 
  ...
  guides(size=FALSE,
         color = guide_legend(override.aes = list(size=4))) + 
  facet_wrap(~gwas, scales = "free_y", nrow = 1) + 
  miller_discrete_scale()
